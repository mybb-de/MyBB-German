<?php
#########################################################
# Deutsches Sprachpaket (Informell)                     #
# Version x.x.x                                         #
# Datum: xx.xx.xxxx                                     #
# MyBB-Version x.x.x                                    #
# Autor: MyBBoard.de | Webseite: http://www.mybboard.de #
# (c) 2005-2011 MyBBoard.de | Alle Rechte vorbehalten!  #
#                                                       #
# Die Lizenz/Nutzungsbedingungen finden Sie in der      #
# beiliegenden Readme.                                  #
#########################################################
 
$l['nav_stats'] = "Foren-Statistiken";

$l['board_stats'] = "Foren-Statistiken";
$l['none'] = "Keine";
$l['nobody'] = "Niemand";
$l['totals'] = "Gesamt";
$l['averages'] = "Durchschnitt";
$l['posts'] = "Beiträge:";
$l['threads'] = "Themen:";
$l['members'] = "Mitglieder:";
$l['ppd'] = "Beiträge pro Tag:";
$l['tpd'] = "Themen pro Tag:";
$l['mpd'] = "Registrierungen pro Tag:";
$l['ppm'] = "Beiträge pro Mitglied:";
$l['rpt'] = "Antworten pro Thema:";
$l['no_posts'] = "Keine";
$l['general'] = "Allgemein";
$l['newest_member'] = "Neuestes Mitglied:";
$l['members_posted'] = "Mitglieder, die Beiträge geschrieben haben:";
$l['todays_top_poster'] = "Heutiger Topposter: <b>{1}</b> (<b>{2}</b> Beiträge)";
$l['popular_forum'] = "Meist benutztes Forum: <b>{1}</b> (<b>{2}</b> Beiträge, <b>{3}</b> Themen)";
$l['most_popular'] = "Interessantes";
$l['most_replied_threads'] = "Themen mit den meisten Antworten";
$l['most_viewed_threads'] = "Themen mit den meisten Ansichten";
$l['not_enough_info_stats'] = "Entschuldigung, aber es gibt in diesem Forum nicht genug Informationen, um Statistiken zu erstellen. Um Statistiken erstellen zu können, benötigt das Forum mindestens einen Benutzer und ein Thema.";
$l['replies'] = "Antworten";
$l['views'] = "Ansichten";

?>
